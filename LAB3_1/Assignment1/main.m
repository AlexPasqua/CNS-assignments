...
%     view(rnn)
    ...
    % TDNN training set
    [tdnn_Xs_tr, tdnn_Xi_tr, tdnn_Ai_tr, tdnn_Ts_tr, ~, ~] = ...
        preparets(tdnn, num2cell(tr_input), num2cell(tr_target));
    % RNN training set
    [rnn_Xs_tr, rnn_Xi_tr, rnn_Ai_tr, rnn_Ts_tr, ~, ~] = ...
        preparets(rnn, num2cell(tr_input), num2cell(tr_target));
    % TDNN validation set
    [tdnn_Xs_val, tdnn_Xi_val, tdnn_Ai_val, tdnn_Ts_val, ~, ~] = ...
        preparets(tdnn, num2cell(tr_input), num2cell(tr_target));
    % RNN validation set
    [rnn_Xs_val, rnn_Xi_val, rnn_Ai_val, rnn_Ts_val, ~, ~] = ...
        preparets(rnn, num2cell(tr_input), num2cell(tr_target));
